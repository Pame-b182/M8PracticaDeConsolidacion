<template>
  <v-container>
    <h1 class="text-center">Electrónica</h1>
    ...
    <v-row v-else>
      ...
      <v-col cols="3" md="4" v-for="producto in electronica" :key="producto.id">
        <v-card :loading="loading" class="mx-auto my-12" max-width="374">
          <v-img height="250" :src="producto.image" contain></v-img>

          <v-card-title class="title">{{
            producto.title.slice(0, 61)
          }}</v-card-title>
          ...
          <v-card-text>
            ...
            <div>{{ producto.description.slice(0, 58) }}</div>
          </v-card-text>
...
export default {
  ...
  methods: {
    ...mapActions(["fetchElectronica", "agregarProductoACarro"]),
